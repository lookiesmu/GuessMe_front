...
class SignUpActivity : AppCompatActivity() {
    ...
    inner class SignUp_Control{
        fun edit_check() : Boolean{
            ...
            val editArray = arrayListOf(R.id.su_et_nickname, R.id.su_et_password, R.id.su_et_passwordcheck)
            ...
            when (state){
                0->{
                    if(jsonObj.getBoolean("success")){
                        ...
                        Toast.makeText(applicationContext, "사용불가능한 닉네임입니다",Toast.LENGTH_SHORT).show()
                }
                1->{
                    if(jsonObj.getBoolean("success")){
    ...
    inner class EditListener : TextWatcher {
        override fun afterTextChanged(s: Editable?) { su_duplecheck.isEnabled = !s.isNullOrEmpty() }
